...
describe('when there is initally two blogs', () => {
    ...
    test('creating new blog post', async () => {
        ...
        assert.equal(updatedList.length, helper.initialBlog.length + 1)
    ...
    test('delete a single blog', async () => {
        ...
        assert.equal(blogsAfter.length, helper.initialBlog.length - 1)
    })
    
    test('updating individual blog', async () => {
        const blogToUpdate = helper.initialBlog[0]._id
        const updatedBlog = {
            likes: 15
        }
        const res = await api
